...
private struct BalanceCard: View { let balance: Int; var body: some View { let other = Person.jamie.rawValue; VStack(alignment: .leading, spacing: 4) { Text(abs(balance).usd).font(.title.bold()); Text(balance == 0 ? "All settled up" : balance > 0 ? "Owed by \(other)" : "You owe \(other)").font(.subheadline) }.foregroundStyle(.primary).frame(maxWidth: .infinity, alignment: .leading).padding(20).background(Color(.systemGray5), in: RoundedRectangle(cornerRadius: 20, style: .continuous)).padding(.vertical, 4) } }
